...
y = np.arange(0, 1, 1.0/np.ceil(n**0.5))
x_, y_ = np.meshgrid(x, y)
z = FrankeFunction(x_, y_)
z += np.random.normal(0, np.mean(z)*0.1, z.shape) # Adding some normal noise
z -= np.mean(z) # Centering the data

x = np.append(x_.ravel().reshape(-1, 1), y_.ravel().reshape(-1, 1), axis=1)
y = z.ravel()
