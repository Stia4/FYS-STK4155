...
class neural_network:
	...
	def create_biases_and_weights(self):
		...
		if (self.initialization == "Random"):
			...
			self.hidden_bias = []
			self.hidden_bias.append(np.zeros((1,self.n_hidden_neurons[0])))
			...
			for i in range(self.n_layers-1):
			    ...
			    self.hidden_bias.append(np.zeros((1,self.n_hidden_neurons[i+1])))
	...
	def backpropagation(self):
		...
		delta_l = self.act_func_out.deriv(self.z[-1])*self.cost_func.deriv(self.y_data, self.a[-1])
...
def run_terrain_test(xy_len = 30, gamma=0.0, eta=0.03, hidden_layers=[30,20,10], epochs=1000, 
	...
	R2 = 1 - np.sum((Z_test - ztilde)**2)/np.sum((Z_test - np.average(Z_test))**2)
	print("R2 score of optimal model : %.4f " % R2)
